...
def dm02_show_iris():
    ...
    #04 通过Seaborn绘制散点图
    sns.lmplot(data=iris_df,x='sepal_length',y='sepal_width',hue='label',fit_reg=True)
    #05.设置标题,显式
    plt.title('iris data')
    plt.tight_layout()
    plt.show()
...
def dm03_split_train_test():
    ...
    x_train,x_test,y_train,y_test=train_test_split(iris_data.data,iris_data.target,test_size=0.2,random_state=17)
    # 3. 打印切割后的结果.
